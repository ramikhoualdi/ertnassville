...
import { signInProperty,resetAllAuthForms, ResetErrorsState } from '../redux/Property/property.actions.js';

const mapState = ({ property }) => ({
  currentProperty: property.currentProperty,
  propertySignInSuccess: property.propertySignInSuccess,
...
const EmailLogin = ({navigation}) => {
  ...
  const handleRegister = async e => {
    ...
    if ( checking_form === 'true' ){
      dispatch(signInProperty({ email, password}));
    }
  };
  const handleForget = () => {
    navigation.navigate('PropertyResetPassword');
  };
